...
const TEXT_FILE_EXTENSIONS = new Set(["txt", "md", "markdown", "csv", "json"]);
...
function isTextLikeFile(file) {
  ...
  return (
    ...
    ["application/json", "application/xml", "application/x-ndjson"].includes(mime) ||
...
async function extractTextFromFile(file) {
  ...
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onload = () => resolve(String(reader.result || ""));
    reader.onerror = () => reject(reader.error || new Error("Unable to extract text"));
...
function analyzeResource({
  ...
  const normalizedResourceContent = normalizeContent(resourceContent).slice(0, 12000);
  const combinedText = [
    resourceTitle,
    description,
    cleanFileName(resourceFile?.name),
  ...
  const summary = buildSummarySentences(description || normalizedResourceContent, fallbackSummary);
