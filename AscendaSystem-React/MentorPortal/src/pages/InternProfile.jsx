...
export default function InternProfile() {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const mentors = Array.from(
          new Set([
            ...accountMentors,
            ...list.map((item) => item.mentor_name).filter(Boolean),
            ...list.map((item) => item.substitute_mentor_name).filter(Boolean),
          ])
  ...
  const handleResponsibleChange = React.useCallback(async (value) => {
    ...
    const mentorAccount = resolveMentor(resolvedValue);
    setIsSavingResponsible(true);
    try {
      const updates = {
        mentor_name: resolvedValue,
        mentor_id: mentorAccount?.id || null,
        mentor_email: mentorAccount?.email || null,
      };
      const updated = await Intern.update(intern.id, updates);
      const nextIntern = updated ?? { ...(intern ?? {}), ...updates };
  ...
  }, [intern, mentorOptions, resolveMentor]);

  const handleSubstituteChange = React.useCallback(async (value) => {
    if (!intern) return;
    ...
    try {
      const updates = {
        substitute_mentor_name: resolvedValue,
        substitute_mentor_id: mentorAccount?.id || null,
        substitute_mentor_email: mentorAccount?.email || null,
      };
      const updated = await Intern.update(intern.id, updates);
      setIntern(updated ?? { ...(intern ?? {}), ...updates });
    } catch (error) {
      console.warn("Failed to update substitute mentor", error);
    } finally {
      setIsSavingResponsible(false);
    }
  }, [intern, resolveMentor]);
  ...
  const substituteValue = intern.substitute_mentor_name ?? "unassigned";
        ...
        <Card className="border-border bg-surface shadow-e1">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-primary text-lg">Designar mentoria</CardTitle>
              {isSavingResponsible && (
            ...
            <p className="text-sm text-muted mt-2">
              Escolha o mentor principal e um substituto para continuidade do acompanhamento.
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <span className="text-xs font-medium uppercase tracking-wide text-muted">Mentor principal</span>
                <Select value={responsibleValue} onValueChange={handleResponsibleChange}>
                  ...
                  <SelectContent className="bg-surface border-border">
                    <SelectItem value="unassigned">Sem mentor definido</SelectItem>
              ...
              <div className="space-y-2">
                <span className="text-xs font-medium uppercase tracking-wide text-muted">Mentor substituto</span>
                <Select value={substituteValue} onValueChange={handleSubstituteChange}>
                  <SelectTrigger className="bg-surface border-border text-primary">
                    <SelectValue placeholder="Selecione um substituto" />
                  </SelectTrigger>
                  <SelectContent className="bg-surface border-border">
                    <SelectItem value="unassigned">Sem substituto definido</SelectItem>
                    {mentorOptions
                      .filter((mentor) => mentor !== responsibleValue)
                      .map((mentor) => (
                        <SelectItem key={mentor} value={mentor}>
                          {mentor}
                        </SelectItem>
            ...
            <form className="space-y-2" onSubmit={handleCustomResponsibleSubmit}>
              <span className="text-xs font-medium uppercase tracking-wide text-muted">Cadastrar novo mentor</span>
